...
def _get_t5_summarizer():
    """Load transformers model lazily so server startup doesn't require torch libs."""
...
def summarize_text(texts):
    ...
    """
    tokenizer, model = _get_t5_summarizer()
...
def fallback_summarize_text(text, max_sentences=10):
    """Simple extractive fallback summary that does not require ML dependencies."""
    ...
    words = [w.lower() for w in cleaned.split() if w.isalpha() and len(w) > 3]
    freq = Counter(words)
    if not freq:
    ...
    for idx, sentence in enumerate(sentences):
        token_list = [w.lower() for w in sentence.split()]
        score = sum(freq.get(tok, 0) for tok in token_list)
        scored.append((score, idx, sentence))

    top = sorted(scored, key=lambda x: x[0], reverse=True)[:max_sentences]
    top_sorted = [item[2] for item in sorted(top, key=lambda x: x[1])]
...
def process_study_material(request, study_material_id):
    ...
    # Summarize using ML model when available, else fallback to an extractive summary.
    try:
        summaries = summarize_text(chunks)
        summary = " ".join(summaries)
    except Exception as exc:
        print(f"Summarizer fallback used: {exc}")
        summary = fallback_summarize_text(text)
...
def generate_questions_with_spacy(text):
    ...
    try:
        ...
        words = [w for w in re.findall(r"[A-Za-z]{4,}", text)][:5]
        if not words:
            return ["What are the key points from these notes?"]
        return [f"Explain {w} in simple terms." for w in words]
...
async def translate_text_with_azure(text, target_languages=['ta', 'ml', 'te', 'kn', 'hi', 'gu', 'bn']):
    try:
        ...
        return {"error": f"Translation dependency missing: {exc}"}
    ...
    for lang in target_languages:
        try:
            ...
            for chunk in text_chunks:
                translated_chunk = translator.translate(chunk, dest=lang)
